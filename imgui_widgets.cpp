...
void ImGui::TextColored(const ImU32& col, const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
...
void ImGui::TextColoredV(const ImU32& col, const char* fmt, va_list args)
{
...
bool ImGui::ImageButtonEx(ImGuiID id, ImTextureID texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec2& padding, const ImU32 bg_col, ImU32 tint_col, ImGuiButtonFlags flags, const ImU32* active_tint_col)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
    ...
    {
        if (!(flags & ImGuiButtonFlags_NoDecorations))
        {
    ...
    if (bg_col != 0u)
        window->DrawList->AddRectFilled(bb.Min + padding, bb.Max - padding, bg_col);
    if (active_tint_col != nullptr && held && hovered)
    {
...
bool ImGui::ImageButton(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImU32 bg_col, const ImU32 tint_col, ImGuiButtonFlags flags, const ImU32* active_tint_col)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (window->SkipItems)
    ...
    return ImageButtonEx(id, user_texture_id, size, uv0, uv1, padding, bg_col, tint_col, flags, active_tint_col);
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImVec2 check_pos = pos + ImVec2(0.0f, ImFloor((total_bb.GetHeight() - square_sz) * 0.5f));
    ...
    {
        RenderCheckMark(window->DrawList, check_bb.Min, check_col, 1.0f);
    }

    if (g.LogEnabled)
        LogRenderedText(&total_bb.Min, *v ? "[x]" : "[ ]");
    if (label_size.x > 0.0f)
        RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, total_bb.Min.y + style.FramePadding.y), label);
...
bool ImGui::RadioButton(const char* label, bool active)
{
    ...
    RenderNavHighlight(total_bb, id);
    window->DrawList->AddImage(ImGui::GetIO().Fonts->TexID, check_bb.Min, check_bb.Min + ImVec2(20, 20), style.RadioButtonUV0, style.RadioButtonUV1, GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button));
    if (active)
    {
        const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));
        window->DrawList->AddCircleFilled(check_bb.Min + ImVec2(10, 10), 3, GetColorU32(ImGuiCol_CheckMark), 8);
    }
...
bool ImGui::MenuItem(const char* label, const char* shortcut, bool selected, bool enabled)
{
    ...
    else
    {
        ...
        if (selected)
            RenderCheckMark(window->DrawList, pos + ImVec2(window->DC.MenuColumns.Pos[2] + extra_w, 0.0f), GetColorU32(enabled ? ImGuiCol_Text : ImGuiCol_TextDisabled), 1.0f);
    }
...
static ImGuiTabItem* ImGui::TabBarScrollingButtons(ImGuiTabBar* tab_bar)
{
    ...
    int select_dir = 0;
    ImU32 arrow_col = g.Style.Colors[ImGuiCol_Text];
    //arrow_col.w *= 0.5f;
    ...
    ImTextureID* user_texture_id, ImVec2 uv0, ImVec2 uv1, const ImU32 tint_col, const ImVec2& imageSize, const ImVec2& imageOffset)
{
    ...
    */
    ...
    if (!tab_bar->bShowLabel || tab->ContentWidth > size.x)
    {
